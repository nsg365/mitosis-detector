...
\title{
Automated Mitosis Detection in Breast
Histopathology: A Deep Learning Framework
Incorporating Macenko Normalization\\ and Faster
R-CNN
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{outputs/06_stain_normalisation_before_after.png}
\caption{Visual comparison of histopathology patches before and after Macenko stain normalization, demonstrating reduction in domain shift.}
\label{fig:stain_norm}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{outputs/stage1_threshold_analysis.png}
\caption{Stage 1 Threshold Analysis showing F1 score and Filtering Rate against classification threshold. The selected threshold ($\tau=0.5087$) optimizes recall.}
\label{fig:stage1_thresh}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{outputs/05_stage2_detection_patches.png}
\caption{Qualitative detection results from Stage 2 Faster R-CNN. Green boxes indicate ground truth, while red boxes indicate model predictions with confidence scores.}
\label{fig:detection_patches}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{outputs/froc_curve.png}
\caption{Free-Response Receiver Operating Characteristic (FROC) curve for the Stage 2 detector, demonstrating high-confidence false positives.}
\label{fig:froc_curve}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{outputs/gradcam_misclassifications.png}
\caption{Grad-CAM visualization of misclassified patches, highlighting regions that drive the model's false positive predictions (e.g., overlapping nuclei).}
\label{fig:gradcam}
\end{figure}
